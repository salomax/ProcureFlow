...
```bash
# Generate supergraph schema (using CLI)
./neotool graphql generate

# Generate supergraph schema with Docker (CI/CD)
...
```bash
# Interactive sync from services to contracts (using CLI)
./neotool graphql sync

# Validate schema consistency
./neotool graphql validate

# Complete workflow: sync, validate, and generate
./neotool graphql all

# Or run script directly
./scripts/cli/commands/graphql/sync-schemas.sh sync
./scripts/cli/commands/graphql/sync-schemas.sh validate
./scripts/cli/commands/graphql/generate-schema.sh
```

### 4. Complete Workflow
For a complete schema management workflow, use the `all` command:
```bash
./neotool graphql all
```

This will:
1. Sync schemas from service modules to contracts
2. Validate schema consistency
3. Generate the supergraph schema
